...
import { Controller, Response, Body, Post, Request, Cookies } from '@decorators/express'
...
export default class SpeechController {
  ...
  async insertNewSpeech <T>(@Request() req, @Response() res, @Body() body): Promise<T> {
    ...
    if (req.session.isLogin) {
      await this.speechService.insertSpeech({ admin: req.session.admin, speechText: text, insterTime: new Date().getTime()})
